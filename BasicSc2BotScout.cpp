 ...
 * @return bool
 */
bool BasicSc2Bot::TryScoutingForAttack(const sc2::Unit *unit_to_scout,
                                       bool refill_enemy_locations) {
    ...
    if (!unexplored_enemy_starting_locations.empty()) {
        ...
        // send
        Actions()->UnitCommand(unit_to_scout, sc2::ABILITY_ID::ATTACK_ATTACK,
                               target);
        return true;
    } else {
        // search random point
        if (ScoutRandom(unit_to_scout, target)) {
            Actions()->UnitCommand(unit_to_scout, sc2::ABILITY_ID::SMART,
                                   target);
            return true;
        }
    }
...
bool BasicSc2Bot::TryScouting(const sc2::Unit &unit_to_scout) {
    ...
    if (!this->unexplored_enemy_starting_locations.empty()) {
        ...
        // start from the back so we can .pop_back() (no pop_front equivalent)
        Actions()->UnitCommand(
            &unit_to_scout, sc2::ABILITY_ID::ATTACK_ATTACK,
            this->unexplored_enemy_starting_locations.back());
        return true;
    }
    return false;
}
...
void BasicSc2Bot::CheckScoutStatus() {
    ...
    if (!this->unexplored_enemy_starting_locations.empty()) {
        ...
        sc2::Units enemy_bases =
            Observation()->GetUnits(sc2::Unit::Enemy, sc2::IsTownHall());
        for (const sc2::Point2D starting_position :
             unexplored_enemy_starting_locations) {
            if (sc2::DistanceSquared2D(starting_position, this->scout->pos) <
                25) {
                unexplored_enemy_starting_locations.pop_back();
                // move to the next base position
                if (!this->unexplored_enemy_starting_locations.empty()) {
                    Actions()->UnitCommand(
                        this->scout, sc2::ABILITY_ID::ATTACK_ATTACK,
                        unexplored_enemy_starting_locations.back());
                }
            }
        }
    }
