...
/**
* @brief Check if conditions to build a factory are met, and build factory if they are
* @return true
*/
...
/**
* @brief Tries find a place to build a structure and issues command to build the structure
    ...
    for (const auto& unit : units) {
        for (const auto& order : unit->orders) {
            if (order.ability_id == ability_type_for_structure) {
                // do not build this strucutre if we have a unit that's already building this type
                return false;
            }
        }
...
* @param expansion_starting_point: The position of a base expansion that we want to build at
* @return true
*/
bool BasicSc2Bot::TryBuildStructure(sc2::ABILITY_ID ability_type_for_structure, sc2::Point2D location, sc2::Point2D expansion_starting_point) {
    ...
    Actions()->UnitCommand(unit_to_build, ability_type_for_structure, expansion_starting_point);
