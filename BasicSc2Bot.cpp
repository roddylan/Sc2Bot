...
void BasicSc2Bot::OnGameStart() {
    ...
    unexplored_enemy_starting_locations =
...
void BasicSc2Bot::OnStep() {
    ...
    // **NOTE** order matters as the amount of minerals we have gets consumed,
    // seige tanks are important to have at each expansion
    TryBuildSupplyDepot();
    LaunchAttack();
    HandleBuild();
    BuildWorkers();
    ...
    sc2::Units tanks =
        obs->GetUnits(sc2::Unit::Alliance::Self,
                      sc2::IsUnits({sc2::UNIT_TYPEID::TERRAN_SIEGETANK,
...
void BasicSc2Bot::RecheckUnitIdle() {
    /*
     * The OnUnitIdle hook that's automatically called by the game is only
     * called ONCE when the unit starts idling. This is an issue for barracks
     * because when OnUnitIdle is called for them but they don't have the
     * resources to train a unit, they won't take an action and OnUnitIdle is
     * never called on them again so they never get a kick to start training
     * when resources are available.
     */
    const sc2::Units &idle_units = Observation()->GetUnits(
        sc2::Unit::Alliance::Self,
        [](const sc2::Unit &unit) { return unit.orders.empty(); });
...
void BasicSc2Bot::OnUnitCreated(const sc2::Unit *unit) {
    ...
    case sc2::UNIT_TYPEID::TERRAN_SUPPLYDEPOT: {
         ...
         * In the future, maybe we can take advantage of raising/lowering them
         * to control movement
         */
    ...
    */
    case sc2::UNIT_TYPEID::TERRAN_MEDIVAC: {
        ...
        sc2::Point2D largest_marine_cluster =
            FindLargestMarineCluster(unit->pos, *unit);
        if (largest_marine_cluster == sc2::Point2D(0, 0))
            return;
        Actions()->UnitCommand(unit, sc2::ABILITY_ID::SMART,
                               largest_marine_cluster);
        break;
    }
...
void BasicSc2Bot::OnUnitDestroyed(const sc2::Unit *unit) {
    ...
    if (unit->unit_type == sc2::UNIT_TYPEID::TERRAN_ORBITALCOMMAND ||
        ...
        if ((Observation()->GetGameLoop() < 13200 &&
             Distance2D(unit->pos, start_location) > 50.0f)) {
            return;
        }
        ...
        sc2::Units marauders = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_MARAUDER));
        filter_units(marauders);

        sc2::Units banshees = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_BANSHEE));
        filter_units(banshees);
        // Command to attack
        Actions()->UnitCommand(marines, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);
        Actions()->UnitCommand(marauders, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);
        Actions()->UnitCommand(vikings, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);
        // Cloak banshees
        for (const auto &banshee : banshees) {
            Actions()->UnitCommand(banshee,
                                   sc2::ABILITY_ID::BEHAVIOR_CLOAKON_BANSHEE);
            Actions()->UnitCommand(banshee, sc2::ABILITY_ID::ATTACK_ATTACK,
                                   unit->pos);
        }

        // Add units to squad and order to attack
        sc2::Units thors = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_THOR));
        filter_units(thors);
        Actions()->UnitCommand(thors, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);

        sc2::Units liberators = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_LIBERATOR));
        filter_units(liberators);
        Actions()->UnitCommand(liberators, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);

        sc2::Units tanks = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_SIEGETANK));
        filter_units(tanks);
        Actions()->UnitCommand(tanks, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);
        ...
        Actions()->UnitCommand(battlecruisers, sc2::ABILITY_ID::ATTACK_ATTACK,
                               unit->pos);

        sc2::Units medivacs = Observation()->GetUnits(
            sc2::Unit::Self, sc2::IsUnit(sc2::UNIT_TYPEID::TERRAN_MEDIVAC));
    ...
    if (unit == this->scout) {
        ...
        for (const sc2::Point2D &position : info.enemy_start_locations) {
            if (sc2::DistanceSquared2D(unit->pos, position) <
                sc2::DistanceSquared2D(unit->pos, closest_base_position)) {
                closest_base_position = position;
            }
        }
        this->enemy_starting_location =
            new sc2::Point2D(closest_base_position.x, closest_base_position.y);
...
void BasicSc2Bot::OnGameEnd() {
    ...
    for (const auto &result : results) {
        ...
        if (result.player_id == pid) {
            ...
            std::cout << "Player [ENEMY] (" << result.player_id << ") | "
                      << player_result << std::endl;
        }
        std::cout << "\n";
    }
    /*
