...
void BasicSc2Bot::BuildWorkers() {
    ...
    for (const auto &base : bases) {
        if (base->unit_type == sc2::UNIT_TYPEID::TERRAN_ORBITALCOMMAND ||
            ...
            if ((CountUnitType(sc2::UNIT_TYPEID::TERRAN_MULE) <
                     bases.size() * n_mules &&
                 base->energy > 50) ||
                base->energy > 75) {
                // find mineral target
                const sc2::Unit *mineral_target =
                    FindNearestMineralPatch(base->pos);
                // if we find a nearby mineral patch
                if (mineral_target) {
                    Actions()->UnitCommand(base,
                                           sc2::ABILITY_ID::EFFECT_CALLDOWNMULE,
                                           mineral_target);
                }
            }
        }
    ...
    for (const auto &base : bases) {
        ...
        if (base->assigned_harvesters < base->ideal_harvesters &&
            base->build_progress == 1 && base->orders.empty()) {
            if (obs->GetMinerals() >= 50 && base->orders.empty() &&
                obs->GetFoodUsed() < 120) {
                sc2::Agent::Actions()->UnitCommand(base,
                                                   sc2::ABILITY_ID::TRAIN_SCV);
            }
        }
    }
}
            ...
            for (const auto &scv : workers) {
                if (!scv->orders.empty()) {
                    ...
                    const sc2::Unit *target =
                        obs->GetUnit(first_order.target_unit_tag);
