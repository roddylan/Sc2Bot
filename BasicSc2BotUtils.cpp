...
// contains implementation for misc. utility functions (finders, is, in, count,
// etc.)
...
bool BasicSc2Bot::LoadBunker(const sc2::Unit *marine) {
    sc2::Units myUnits = Observation()->GetUnits(
        sc2::Unit::Alliance::Self, IsUnit(sc2::UNIT_TYPEID::TERRAN_BUNKER));
...
int BasicSc2Bot::CountNearbySeigeTanks(const sc2::Unit *factory) {
    sc2::Units seige_tanks = Observation()->GetUnits(
        sc2::Unit::Alliance::Self, IsUnit(sc2::UNIT_TYPEID::TERRAN_SIEGETANK));
    int count = 0;
...
size_t BasicSc2Bot::CountUnitType(sc2::UNIT_TYPEID unit_type) {
    return Observation()
        ->GetUnits(sc2::Unit::Alliance::Self, IsUnit(unit_type))
        .size();
}
 ...
 */
const sc2::Unit *
BasicSc2Bot::FindNearestMineralPatch(const sc2::Point2D &start) {
    ...
    for (const auto &u : units) {
        if (u->unit_type == sc2::UNIT_TYPEID::NEUTRAL_MINERALFIELD ||
            u->unit_type == sc2::UNIT_TYPEID::NATURALMINERALS ||
            u->unit_type == sc2::UNIT_TYPEID::MINERALCRYSTAL ||
            u->unit_type == sc2::UNIT_TYPEID::NEUTRAL_RICHMINERALFIELD) {
 ...
 */
const sc2::Unit *
BasicSc2Bot::FindNearestVespeneGeyser(const sc2::Point2D &start) {
    sc2::Units units = Observation()->GetUnits(sc2::Unit::Alliance::Neutral);
    float distance = std::numeric_limits<float>::max();
    const sc2::Unit *target = nullptr;
    for (const auto &u : units) {
        if (u->unit_type == sc2::UNIT_TYPEID::NEUTRAL_VESPENEGEYSER ||
            u->unit_type == sc2::UNIT_TYPEID::NEUTRAL_SPACEPLATFORMGEYSER) {
            float d = sc2::DistanceSquared2D(u->pos, start);
            if (d < distance) {
                distance = d;
                target = u;
 ...
 * @return const sc2::Point2D
 */
const sc2::Point2D
BasicSc2Bot::FindNearestCommandCenter(const sc2::Point2D &start,
                                      bool not_start_location) {
    ...
    for (const auto &base : bases) {
        if (base->unit_type == sc2::UNIT_TYPEID::TERRAN_ORBITALCOMMAND ||
            ...
            if (not_start_location && base->pos == start_location)
                continue;
...
const sc2::Point2D BasicSc2Bot::FindNearestRefinery(const sc2::Point2D &start) {
    ...
    sc2::Units refineries = Observation()->GetUnits(
 ...
 */
int BasicSc2Bot::MarineClusterSize(const sc2::Unit *marine,
...
const sc2::Unit *BasicSc2Bot::FindInjuredMarine() {
    const sc2::Units marines = Observation()->GetUnits(
        sc2::Unit::Alliance::Self, IsUnit(sc2::UNIT_TYPEID::TERRAN_MARINE));
 ...
 *
 * @param start
 * @return const sc2::Units
 */
const sc2::Units
BasicSc2Bot::SortMedivacsAccordingToDistance(const sc2::Point2D start) {
    sc2::Units medivacs = Observation()->GetUnits(
        sc2::Unit::Alliance::Self, IsUnit(sc2::UNIT_TYPEID::TERRAN_MEDIVAC));
    std::sort(medivacs.begin(), medivacs.end(),
              [&start](const sc2::Unit *unit_a, const sc2::Unit *unit_b) {
 ...
 */
const sc2::Point2D
BasicSc2Bot::FindLargestMarineCluster(const sc2::Point2D &start,
                                      const sc2::Unit &unit) {
    const sc2::Units marines = Observation()->GetUnits(
        sc2::Unit::Alliance::Self, IsUnit(sc2::UNIT_TYPEID::TERRAN_MARINE));
    ...
    for (const auto &marine : marines) {
        ...
        if (cluster_size > largest_cluster_size &&
            sc2::Distance2D(closest_medivac->pos, marine->pos) > 15.0f) {
...
/**
 * @brief Gets an SCV that is currently gathering minerals/gas
 *
 * @return const sc2::Unit*
 */
const sc2::Unit *BasicSc2Bot::GetGatheringScv() {
    const sc2::Units &gathering_scv_units = Observation()->GetUnits(
...
/*
 * @brief Functor that implements < for a Point2D so that we can holds points in
 * a std::set in FindPlaceablePositionNear
 */
struct Point2DLt {
...
/**
 * @brief Finds a location to place a structure
 *
 * @param starting_point
 * @param ability_to_place_building
 * @return sc2::Point2D
 */
sc2::Point2D BasicSc2Bot::FindPlaceablePositionNear(
    const sc2::Point2D &starting_point,
    const sc2::ABILITY_ID &ability_to_place_building) {
    // Finds a location to place a building at using a triangle with its base as
    // a line between 2 nearest structures to starting_point

    // Get all existing structures
    sc2::Units all_buildings = Observation()->GetUnits(
        sc2::Unit::Alliance::Self,
        sc2::IsUnits({sc2::UNIT_TYPEID::TERRAN_COMMANDCENTER,
    ...
    /*
        ...
        * via ChatGPT: "im trying to use triangles to nicely space out my
       structures from another, take 2 and make a triangle out of them using the
       line between the structures"
        * "i want you to make sure it finds a point it has to keep finding one
       until the placement condition is true"
    */
    for (int i = 0; i < all_buildings.size(); i++) {
        if ((i + 1) < all_buildings.size()) {
            ...
            sc2::Point2D midpoint = sc2::Point2D((a->pos.x + b->pos.x) / 2,
                                                 (a->pos.y + b->pos.y) / 2);
            // Drection vector from building a to b
            sc2::Point2D direction =
                sc2::Point2D(b->pos.x - a->pos.x, b->pos.y - a->pos.y);
            float magnitude = std::sqrt(direction.x * direction.x +
                                        direction.y * direction.y);
            sc2::Point2D normalized_direction =
                sc2::Point2D(direction.x / magnitude, direction.y / magnitude);

            // First perpendicular vector
            sc2::Point2D perpendicular =
            ...
            while (true) {
                ...
                if (Query()->Placement(ability_to_place_building, new_point,
                                       placing_unit)) {
                    // Check for wiggle room for techlab/reactor
                    bool can_wiggle_left = Query()->Placement(
                        ability_to_place_building,
                        sc2::Point2D(new_point.x - 2, new_point.y),
                        placing_unit);
                    bool can_wiggle_right = Query()->Placement(
                        ability_to_place_building,
                        sc2::Point2D(new_point.x + 2, new_point.y),
                        placing_unit);
                ...
                if (height_of_triangle > 20.0f) {
                    ...
                    perpendicular = (direction_changes % 2 == 0)
                                        // Rotate other way
                                        ? sc2::Point2D(normalized_direction.y,
                    ...
                    // Stop searching if we cannot find a point before threshold
                    // -> return error point
 ...
 * @param mineral allow selecting workers holding minerals (use if implement
 * mineral walk)
 * @return const sc2::Unit*
 */
const sc2::Unit *BasicSc2Bot::FindNearestWorker(const sc2::Point2D &pos,
                                                bool is_busy, bool mineral) {
    const sc2::ObservationInterface *obs = Observation();
...
size_t BasicSc2Bot::CountUnitTotal(const sc2::ObservationInterface *obs,
    ...
    // count existing
    size_t existing =
        obs->GetUnits(sc2::Unit::Alliance::Self, sc2::IsUnit(unit_type)).size();
    // size_t in_production = 0;

    // sc2::Units production_units = obs->GetUnits(sc2::Unit::Alliance::Self,
    // sc2::IsUnit(prod_unit));
    size_t in_production =
        obs->GetUnits(sc2::Unit::Alliance::Self,
                      [&prod_unit, &ability](const sc2::Unit &unit) {
 ...
 */
size_t
BasicSc2Bot::CountUnitTotal(const sc2::ObservationInterface *obs,
                            const std::vector<sc2::UNIT_TYPEID> &unit_type,
                            const std::vector<sc2::UNIT_TYPEID> &prod_unit,
                            sc2::ABILITY_ID ability) {
    // count existing
    size_t existing =
        obs->GetUnits(sc2::Unit::Alliance::Self, sc2::IsUnits(unit_type))
            .size();
    size_t in_production = 0;

    sc2::Units production_units =
        obs->GetUnits(sc2::Unit::Alliance::Self, sc2::IsUnits(prod_unit));
 ...
 */
void BasicSc2Bot::SquadSplit(const size_t &split_sz, sc2::Units &units,
                             sc2::Units &squad) {
