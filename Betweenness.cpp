...
vector<sc2::Point2DI> directions = {sc2::Point2DI(0, 1), sc2::Point2DI(0, -1),
                                    sc2::Point2DI(-1, 0), sc2::Point2DI(1, 0)};
...
bool isValid(sc2::Point2DI p, sc2::ImageData data) {
    ...
    return (p.x >= 0 && p.x < rows && p.y >= 0 && p.y < cols &&
            GetDataValueBit(data, p.x, p.y) == 1);
}
...
struct CompareBetweenness {
    bool operator()(const std::pair<sc2::Point2DI, double> &a,
                    const std::pair<sc2::Point2DI, double> &b) {
...
vector<sc2::Point2DI> SampleNodes(int numSamples, int rows, int cols,
                                  sc2::ImageData data) {
 ...
 */
void MonteCarloBrandesBetweenness(sc2::ImageData data,
                                  vector<vector<double>> &betweenness,
                                  int numSamples) {
    int rows = data.height;
    int cols = data.width;
    ...
    vector<sc2::Point2DI> sampledNodes =
        SampleNodes(numSamples, rows, cols, data);
    ...
    for (const auto &src : sampledNodes) {
        ...
        vector<vector<vector<sc2::Point2DI>>> predecessors(
            rows, vector<vector<sc2::Point2DI>>(cols));
        ...
        while (!stackOrder.empty()) {
            ...
            for (sc2::Point2DI pred : predecessors[u.x][u.y]) {
                ...
                dependency[pred.x][pred.y] +=
                    (1 + dependency[u.x][u.y]) * ratio;
            }
 ...
 * @param data: the bitmap
 * @return tuple containing the point with highest betweenness value and its
 * betweenness value
 */
tuple<sc2::Point2DI, double> CalculateBetweenness(sc2::ImageData data) {
    ...
    std::priority_queue<std::pair<sc2::Point2DI, double>,
    ...
    std::reverse(betweennessList.begin(),
                 betweennessList.end()); // to sort in descending order

    // Return the top point with the highest betweenness centrality
    return std::make_tuple(
        sc2::Point2DI(betweennessList[0].first.x, betweennessList[0].first.y),
        betweennessList[0].second);
}
